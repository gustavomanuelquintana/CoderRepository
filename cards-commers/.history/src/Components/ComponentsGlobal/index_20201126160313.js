...
export const ButtonDanger = ({ text,onClick,type }) => (
    <button className="btn btn-danger mr-2 ml-2" onClick={onClick}>{text}</button>
    );

    export const Input = ({ name,placeHolder,onChange,type }) => (
        <input type={type} class="form-control" onChange={onChange} placeHolder={placeHolder} name={name}/>
        );
