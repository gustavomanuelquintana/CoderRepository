...
export const sumItems = cartItems => {
    let itemCount = cartItems.reduce((total, product) => total1 + product.cantidad, 0);
    let total = cartItems.reduce((total, product) => total1 + product.item.precio * product.cantidad, 0).toFixed(2);
...
function onAddItem(state,item){
    ...
    if(index === -1){
        ...
        return {
            ...state,
            ...sumItems(updateCart.item),
            cartItems:[...updateCart.item]
        }
