    ...
    const handleAdd = (counter) => {
      return ({stock}) => {
        if (stock <=0){
          ...
          setCompra({isValid: true,cantidad:counter,id: itemcart.item.id, productName : itemcart.item.productName, image: itemcart.item.image, precio: itemcart.item.precio})
         handleBuy();
        }
      }
    }
