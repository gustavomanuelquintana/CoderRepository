...
const amountPrice = (cart,total,setTotal) => {
  let totalPrice = 0;
  let price = cart[0][0].forEach( value =>{totalPrice+= (value.item.precio * value.cantidad)}); 
  setTotal(totalPrice)
}

const count= (cart,countercart,setCountercart) => {
  let counter = 0;
  let itemCounter = cart[0][0].forEach( value => counter += value.cantidad);
...
const Cart = () => {
    ...
    <Row sm="12 mt-4">
        <Col sm="8">
          {
            cart[0][0].length > 0 ?
            cart[0][0].map(product => <Cartdetail key={product.id} product={product} title={"Section Car"}/>)
