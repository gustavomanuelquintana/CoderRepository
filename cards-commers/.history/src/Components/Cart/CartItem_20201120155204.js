...
const amountPrice = (cart,total,setTotal) => {
  let totalPrice = 0;
  let price = cart.forEach( value =>{totalPrice+= (value.item.item.precio * value.cantidad)}); 
  setTotal(totalPrice)
}
...
const Cart = () => {
    ...
    <Row sm="12 mt-4">
        <Col sm="8">
          {
            cart.length > 0 ?
            cart.map(product => <Cartdetail key={product.id} count={countercart} product={product.item} title={"Section Car"}/>)
