...
const Cart = () => {
    ...
    const mapStateToProps = state => {
      const total = state.cart.reduce((total, product) => total + product.item.precio * product.cantidad, 0)
      return total;
    }
            ...
            cart.length > 0 &&
             <div className="col-sm-8 divTest">
               <div className="col-sm-6 totalItem">
                 <h6>Total Items</h6>
        <p className="countItem">{cartCount}</p>
               </div>
               <div class="col-sm-6 totalItem">
                 <h6>Total Precio</h6>
        <p className="count">{mapStateToProps(state)}}</p>
               </div>
               <hr />
